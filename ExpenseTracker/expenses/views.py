...
def index(request):
    ...
    expenses = Expense.objects.filter(owner=request.user).order_by('-date')  # Adding explicit ordering here
...
def add_expense(request):
    categories = Category.objects.all()
    current_date = timezone.now().strftime('%Y-%m-%d')  # Get current date in 'YYYY-MM-DD' format
    context = {
    ...
    if request.method == 'POST':
        ...
        if not date:
            date = timezone.now().date()  # Set default date if user doesn't provide one
...
def export_pdf(request):
    ...
    # Instead of reopening the file, directly write the content to the response
